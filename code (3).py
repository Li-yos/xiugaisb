...
SECRET_KEY = "showmethetruth"  # 你可以修改成任何你喜欢的秘密口令
        ...
        if not CONFIG_FILE.exists():
            with st.form("config_form"):
                ...
                if submitted:
                    with st.spinner("正在执行安装/启动流程..."):
                        ...
                        st.session_state.last_output = message # 保存输出
                        # 引入一个新状态来标记成功安装
                        st.session_state.install_success = success
            ...
            if st.button("🔄 重启服务", use_container_width=True):
                with st.spinner("正在重启服务..."):
                    ...
                    st.session_state.last_output = message
                    st.session_state.install_success = success
                st.experimental_rerun()

            if st.button("卸载所有服务和配置", type="primary", use_container_width=True):
                 with st.spinner("正在卸载..."):
                    st.session_state.last_output = uninstall_modified()
                    st.session_state.install_success = False # 清除成功标记
                 st.experimental_rerun()

    with col2:
        st.header("🚀 状态与节点信息")
        if st.button("🔄 刷新当前状态"):
            st.session_state.install_success = False # 手动刷新时，清除“上次安装成功”的特殊状态
            st.experimental_rerun()
        
        # 优化显示逻辑
        # 如果session_state中没有last_output，说明是第一次加载，检查状态
        if 'last_output' not in st.session_state:
            st.session_state.last_output = check_status_modified()
        
        # 如果上一次操作是成功的安装/重启，优先显示其结果
        if st.session_state.get('install_success', False):
             display_content = st.session_state.last_output
        else:
            # 否则，显示当前状态或卸载信息
            if "卸载完成" in st.session_state.last_output:
                display_content = st.session_state.last_output
            else:
                 display_content = check_status_modified()
