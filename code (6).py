...
def generate_vmess_link(config):
    ...
    vmess_obj = {
        ...
        "aid": "0",
        "net": "ws",
        # "type": "none",  # <-- 已移除! 在'net'为'ws'时，不应有此字段。
        "host": config.get("host", ""),
        "path": config.get("path", ""),
        "tls": "tls",
        "sni": config.get("sni", "")
    }
    vmess_str = json.dumps(vmess_obj, sort_keys=True, separators=(',', ':'))
...
def generate_links_modified(domain, port_vm_ws, uuid_str):
    output = []
    # ----------------- 关键修改点 2 -----------------
    ws_path = f"/{uuid_str[:8]}-vm"  # <-- 已移除路径中的查询参数 ?ed=2048
...
def install_or_start_modified(uuid_str_in, port_vm_ws_in, custom_domain_in, argo_token_in):
    try:
        ...
        # ----------------- 关键修改点 3 -----------------
        ws_path = f"/{uuid_str[:8]}-vm" # <-- 同样，移除查询参数
        # 修改sing-box配置，移除early_data相关设置以匹配路径
...
# --- UI 渲染函数 (与修复AttributeError后的版本相同) ---
def render_real_ui():
