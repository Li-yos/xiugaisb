...
def generate_vmess_link(config):
    vmess_obj = {"v": "2", "ps": config.get("ps", "ArgoSB-TLS"), "add": config.get("add", ""), "port": str(config.get("port", "443")), "id": config.get("id", ""), "aid": "0", "scy": "auto", "net": "ws", "type": "none", "host": config.get("host", ""), "path": config.get("path", "/"), "tls": "tls", "sni": config.get("sni", "")}
...
def generate_links_modified(domain, port_vm_ws, uuid_str):
    ...
    output = ["✅ **服务启动成功! (仅TLS节点)**\n---\n", f"**域名 (Domain):** `{domain}`\n**UUID:** `{uuid_str}`\n**本地Vmess端口:** `{port_vm_ws}`\n**WebSocket路径:** `{ws_path}`\n---\n", "**所有节点链接 (可直接复制):**"] + all_links
...
def _start_services():
    ...
    # 核心修复: 确保 cloudflared 命令最简化，不带任何路径
    cf_cmd = [str(cloudflared_path), 'tunnel', '--no-autoupdate', 'run', '--token', argo_token] if argo_token else [str(cloudflared_path), 'tunnel', '--url', f'http://localhost:{port_vm_ws}']
...
# ... (其余所有函数，包括 _stop_services, uninstall_modified, check_status_modified, health_check_and_heal, UI渲染等，都保持不变) ...
def _stop_services():
    ...
    try:
        ...
        if not sb_pid or subprocess.run(f"ps -p {sb_pid}", shell=True, capture_output=True).returncode != 0:
            ...
            with open(SB_LOG_FILE, "a") as sb_log:
                sb_process = subprocess.Popen([str(INSTALL_DIR / "sing-box"), 'run', '-c', 'sb.json'], cwd=INSTALL_DIR, stdout=sb_log, stderr=subprocess.STDOUT)
            SB_PID_FILE.write_text(str(sb_process.pid))
        ...
        if not cf_pid or subprocess.run(f"ps -p {cf_pid}", shell=True, capture_output=True).returncode != 0:
            ...
            config = json.loads(CONFIG_FILE.read_text())
            port_vm_ws, argo_token = config["port_vm_ws"], config.get("argo_token")
            cf_cmd = [str(INSTALL_DIR / "cloudflared"), 'tunnel', '--no-autoupdate', 'run', '--token', argo_token] if argo_token else [str(INSTALL_DIR / "cloudflared"), 'tunnel', '--url', f'http://localhost:{port_vm_ws}']
...
def render_password_setup_ui():
    ...
    with st.form("password_setup_form"):
        ...
        if submitted:
            if not secret_key_in or not node_password_in: st.error("所有密码字段都不能为空！")
    ...
    with col1:
        ...
        if not CONFIG_FILE.exists():
            with st.form("config_form"):
                ...
                port_vm_ws_in = st.number_input("Vmess 本地端口", min_value=0, max_value=65535, value=0, help="0则随机生成")
                st.markdown("---")
            ...
            st.success("已检测到固定配置。")
            if st.button("🔄 重启服务 (重新生成配置)", use_container_width=True):
                with st.spinner("正在重启服务..."):
                    uninstall_modified()
                    time.sleep(2)
                    success, message = install_or_start_modified(None, None, None, None)
                    st.session_state.output = "✅ 服务重启并重新生成配置文件成功。" if success else f"❌ 重启失败: {message}"
                    st.rerun()
...
def render_node_info_page():
    ...
    if SINGBOX_CONFIG_FILE.exists():
        with st.expander("📥 下载 sing-box 客户端配置文件 (推荐)"):
            ...
            st.caption("提示: 此配置文件已包含所有节点，并可自动更新。")
    ...
    else: st.warning("客户端配置文件不存在，请尝试重启服务以重新生成。")
    st.markdown("---")
    ...
    if st.button("查询天气"):
        if city == SECRET_KEY:
            ...
            with st.spinner(f"正在查询 {city} 的天气..."):
                time.sleep(2); st.error(random.choice([f"查询 **{city}** 失败。", f"API密钥过期。", f"网络超时。"]))
